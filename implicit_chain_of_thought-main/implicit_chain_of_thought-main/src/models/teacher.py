...
from transformers import LlamaForCausalLM  # Import LLaMA model
...
class Teacher(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.base_model = LlamaForCausalLM.from_pretrained(config.base_model)  # Updated model
        self.tokenizer = AutoTokenizer.from_pretrained(config.tokenizer_name)
        num_layers = self.base_model.config.num_hidden_layers  # Ensure this matches LLaMA
    ...
    def forward(self, input_ids):
        outputs = self.base_model(input_ids=input_ids, output_hidden_states=True)  # Get hidden states
        return outputs
    ...
    def compute_positions_to_extract_per_layer(self, subset, delta, first_sep_positions, second_sep_positions):
        ...
        for batch_id in range(batch_size):
            ...
            elif subset in ['first_column', 'last_column']:
                delta = 0
            else:
    ...
    def extract_states(self, input_ids, delta, subset='diagonal'):
        ...
        for i, hidden_state in enumerate(hidden_states):
            if subset in ['diagonal', 'first_column', 'last_column']:
